"""
GpuReduc1D_ranges - GPU 1D reduction with ranges support and runtime backend selection.
...
class GpuReduc1D_ranges_Cuda(MapReduce, Cuda_link_compile):
    """GpuReduc1D_ranges using CUDA/CMake backend (for JAX)."""
    ...
    def get_code(self):
        super().get_code()
        _generate_gpu_reduc1d_ranges_code(self)


class GpuReduc1D_ranges_Nvrtc(MapReduce, Nvrtc_link_compile):
    """GpuReduc1D_ranges using NVRTC backend (for PyTorch/NumPy)."""
...
def _generate_gpu_reduc1d_ranges_code(self):
    """Shared code generation logic for GpuReduc1D_ranges."""
...
class GpuReduc1D_ranges:
    """
    Factory class for GPU 1D reduction with ranges and runtime backend selection.
    """
    ...
    def __new__(cls, *args, lang=None):
        if _use_cuda_backend(lang):
            return GpuReduc1D_ranges_Cuda(*args, lang=lang)
        else:
            return GpuReduc1D_ranges_Nvrtc(*args, lang=lang)
