"""
GpuReduc2D - GPU 2D reduction with runtime backend selection.
...
class GpuReduc2D_Cuda(MapReduce, Cuda_link_compile):
    """GpuReduc2D using CUDA/CMake backend (for JAX)."""
    ...
    def get_code(self):
        super().get_code()
        _generate_gpu_reduc2d_code(self)


class GpuReduc2D_Nvrtc(MapReduce, Nvrtc_link_compile):
    """GpuReduc2D using NVRTC backend (for PyTorch/NumPy)."""
...
def _generate_gpu_reduc2d_code(self):
    """Shared code generation logic for GpuReduc2D."""
    i = self.i
    j = self.j
...
class GpuReduc2D:
    """
    Factory class for GPU 2D reduction with runtime backend selection.
    """
