...
extern "C" int launch_keops_kernel(
    ...
    static thread_local std::vector<char> host_buffer;
...
extern "C" int launch_keops_kernel(
    ...
    // NEW: Use XLA scratch instead of tls_workspace
    char* device_ptr = (char*)scratch_ptr;
    float** args_d = (float**)device_ptr;

    cudaMemcpyAsync(args_d, args, sizeof(float*) * (nvi + nvj + nvp), cudaMemcpyHostToDevice, stream);
