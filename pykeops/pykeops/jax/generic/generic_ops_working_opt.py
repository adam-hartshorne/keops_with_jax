...
def make_keops_jax_op(formula: str, aliases: Tuple[str, ...], reduction_op: str, axis: int, dtype_str: str, enable_vjp: bool = True, max_order: int = 2):
    ...
    def keops_jax_op_bwd(residuals, g):
        ...
        eta_dim = g.shape[-1]
        cache_key = (eta_dim, tuple(arg.shape for arg in args))
        ...
        for i, grad_info in enumerate(compiled_grads):
            ...
            if var_cat != 2:
                ...
                grad_i = raw_grad + (jnp.array(0, dtype=args[i].dtype) * args[i])
            else:
...
def get_registry_info():
    return {'note': 'Fully optimized: C++ scratch calculation + pre-compiled gradients'}